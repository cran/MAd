...
CatModf <-  function(meta,  mod) {
  ...
  mod.sig$L.95ci <- mod.sig$ES-1.96*mod.sig$Std.Err
  mod.sig$U.95ci  <- mod.sig$ES+1.96*mod.sig$Std.Err
...
MAreg1 <- function(meta, mod, method="random") {  # Single predictor meta-regression
  ...
  if(method == "fixed") {
    ...
    t.crit <- qt(.975,  df)
    newSE <- round(summary(reg)$coef[, 2]/sqrt(ms.error), 6)  # 15.20 
    Bs <- round(summary(reg)$coef[, 1], 6)
